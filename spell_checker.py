...
def fetch_words(read_mode):
    ...
    words_from_dictionary = [
        word.strip() for word in open("words.txt").readlines()
    ]
...
# FIXED: Access mode was incorrect
WORDS = fetch_words("r")
LETTERS = list(ascii_lowercase) + ["ñ", "á", "é", "í", "ó", "ú"]

# Save all the words from the txt file
WORDS_INDEX = {}

# Loop all the words from the txt and pushin it to WORDS_INDEX
# FIXED: The condition statement was incorrect
for word in WORDS:
...
# FIXED:
# 1. one_length_edit was not called
# 2. pass wrong arg to filter_real_words on single_word_possible_corrections
# 3. Wrong order of the conditions
def possible_corrections(word):

    single_word_possible_corrections = filter_real_words(word)
    one_length_edit_possible_corrections = filter_real_words(
        one_length_edit(word)
    )
    two_lenght_edit_possible_corrections = filter_real_words(
        two_lenght_edit(word)
    )
    no_correction_at_all = word

    #print(f"Single word: {single_word_possible_corrections}")
    #print(f"One Length word: {one_length_edit_possible_corrections}")
    #print(f"Two Length word: {two_lenght_edit_possible_corrections}")
    #print(f"No correction: {no_correction_at_all}")
...
# FIXED: Random module was not imported
...
def test_spell_check_sentence():
    ...
    assert (
        "en mi escuela no enseñan a escribir ni a le" == spell_check_sentence(
            sentence
        )
    )
